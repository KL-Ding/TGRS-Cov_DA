...
from UDA.utils.serialization import json_load
from UDA.dataset.base_dataset import BaseDataset
...
class TargetDataSet(BaseDataset):
    ...
    def get_metadata(self, name):
        img_name = name +".tif"
        img_file = self.root / 'image'/ img_name#self.set /
        label_name = name + ".tif"
        label_file = self.root / 'label' / label_name#self.set /
